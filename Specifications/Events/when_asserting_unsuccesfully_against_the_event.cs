...
namespace Dolittle.Machine.Specifications.Events
{
    ...
    public class when_asserting_unsuccesfully_against_the_event : an_aggregate_root_with_uncommitted_events
    {
        ...
        Because of = () => 
        {
            an_event_at_beginning_with_incorrect_values = Catch.Exception(() => aggregate_root.ShouldHaveEvent<AnEvent>().AtBeginning()
                                                            .WithValues(_ => _.AString == fourth_event.AString, _ => _.AnInt == fourth_event.AnInt));
            another_event_at_end_with_incorrect_values = Catch.Exception(() => aggregate_root.ShouldHaveEvent<AnotherEvent>().AtEnd()
